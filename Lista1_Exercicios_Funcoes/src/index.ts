...
function encontrarMaiorNumero(lista: number[]): number {
    if (lista.length === 0) {
...
function verificarParOuImpar(numero: number): boolean {
    return numero % 2 === 0;
}
...
const string2: string = "O incrivel hulk";
...
function calcularValorComPorcentagem(valor: number, porcentagem: number): number {
    const valorAcrescido = valor + (valor * (porcentagem / 100));
    return valorAcrescido;
}

// Exemplo:
const valorOriginal: number = 100;
const porcentagemAcrescimo: number = 20;
...
function inverterOrdemDasPalavras(frase: string): string {
...
// Exemplo:
const fraseOriginal: string = "Você quer brincar na neve?";
...
function somaDosPares(numeros: number[]): number {
    let soma = 0;
